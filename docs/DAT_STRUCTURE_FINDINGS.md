...
**Answers an open question flagged in `docs/AIRCRAFT_FORMAT_NOTES.md`** ("whether the hangar-quality variant shares this same slot layout") - **it does not**. A hangar aircraft's package is much simpler than the flight model: just one container holding a small mesh/texture package. No P3D, no gun/missile parameters, no separate SpW package - hangar display doesn't need any of that.

**Independently confirmed via CLI casting** (see `docs/ACM_FORMAT_NOTES.md`'s "Hangar-display mesh packages" section) - the real shape is 10 children: 3-4 ACM meshes, 2-3 sixteen-byte placeholder slots, 3 GIM textures, and a trailing small `Asset` that looks like inert debug-heap padding (`0xCC` fill bytes) rather than real data. This is close to but not exactly d0g's original description ("2 ACM LODs, a landing gear model, a fuel tank ACM" = 4 meshes) - among all 36 distinct playable aircraft checked, only Draken's package has that 4th ACM mesh (a fuel tank, sized identically to its flight-model fuel tank); every other one, including MPBM and the TLS special weapon mount (see below), has only 3 real meshes and a placeholder in the fuel-tank slot. The type previously named `aircraft_hangar_dat`/`DatAircraftHangar` was renamed to `hangar_display_dat`/`DatHangarDisplay` once this shape was confirmed to also cover genuinely non-aircraft subjects.

...
**Answers an open question flagged in `docs/AIRCRAFT_FORMAT_NOTES.md`** ("whether `DatAircraftHangar` shares this same slot layout") - **it does not**. A hangar aircraft's package is much simpler than the flight model: just one container holding 2 ACM LODs, a landing gear model, a fuel tank ACM, 3 GIM textures (512x512, 256x256, cast shadow), and a null/padding file. No P3D, no gun/missile parameters, no separate SpW package - hangar display doesn't need any of that.
